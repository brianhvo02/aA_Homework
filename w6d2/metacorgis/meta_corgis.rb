...
class MetaCorgiSnacks
  def initialize(snack_box, box_id)
    @snack_box = snack_box
    @box_id = box_id
    snack = snack_box.methods.grep(/^get_(.*)_info$/) { MetaCorgiSnacks.define_snack $1 }[box_id]
    MetaCorgiSnacks.define_snack(snack)
  end
  ...
  def self.define_snack(name)
    define_method("get_#{name}_info") do
      @snack_box.send("get_#{name}_info", @box_id)
    end

    define_method("get_#{name}_tastiness") do
      @snack_box.send("get_#{name}_tastiness", @box_id)
    end

    define_method(name) do
      info = send("get_#{name}_info")
      tastiness = send("get_#{name}_tastiness")
      result = "#{name.capitalize}: #{info}: #{tastiness} "
      tastiness > 30 ? "* #{result}" : result
    end
  end
